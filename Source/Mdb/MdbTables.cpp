...
void AccountTable::Dump(const char* dir)
{
	...
	fprintf(dumpFile, "BrokerID, AccountID, OrgID, AccountName, AccountClass, AccountType, PrimaryAccountID, AccountStatus, CurrencyID, DeleteFlag\n");
	char buff[4096] = { 0 };
	for (auto it = m_PrimaryKey.m_Index.begin(); it != m_PrimaryKey.m_Index.end(); ++it)
	{
		fprintf(dumpFile, "%d, %s, %s, %s, %c, %c, %s, %c, %s, %d\n",
			(*it)->BrokerID, (*it)->AccountID, (*it)->OrgID, (*it)->AccountName, (*it)->AccountClass, (*it)->AccountType, (*it)->PrimaryAccountID, (*it)->AccountStatus, (*it)->CurrencyID, (*it)->DeleteFlag);
	}
	fclose(dumpFile);
}
